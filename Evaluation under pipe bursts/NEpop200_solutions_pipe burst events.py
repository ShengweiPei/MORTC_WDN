# -*- coding: utf-8 -*-
"""
Created on Sat Feb 17 23:52:38 2024
...
def Pumpsolution_selection(scenario_num, solution_num):
    ...
    normal_resilience = data_sheet_NN.row_values(solution_num)[2]
    pump_solutions = data_sheet_NN.row_values(solution_num)[5:29]
...
def Network_params(scenario_num, solution_num):
    
    ### extract network_params
    path_NN = "E:/MORTC/Anytown/Anytown_NE_1h/population 200" +  "/" + "network_params.xlsx"
    workbook_NN = xlrd.open_workbook(path_NN)
    data_sheet_NN = workbook_NN.sheet_by_index(0) #通过索引获取该列数据
    rowNum_NN = data_sheet_NN.nrows
    colNum_NN = data_sheet_NN.ncols
    
    network_params = data_sheet_NN.col_values(solution_num)
    
    path_NN = "E:/MORTC/Anytown/Anytown_NE_1h/Results_on_default scenario pop200/" +  "/" + str(scenario_num) + ".xlsx"
    workbook_NN = xlrd.open_workbook(path_NN)
    data_sheet_NN = workbook_NN.sheet_by_index(0) #通过索引获取该列数据
    rowNum_NN = data_sheet_NN.nrows
    colNum_NN = data_sheet_NN.ncols
    normal_resilience = data_sheet_NN.col_values(1)[solution_num]
    
    return network_params, normal_resilience

def EPANET_Interaction(burst_pipe_num, scenario_num, kkk, solution_num):
   
    
    network_params, normal_resilience = Network_params(scenario_num, solution_num)
    params = np.array(network_params)
    ...
    for period in range (EP_LEN):
        ...
        PUMP_SOLUTION.append(pump_solution)
    ...
    return Operational_cost, normal_resilience, constraint_cost, demand_satisfaction, PUMP_SOLUTION, switch_times
...
def Performance_under_pipe_bursts():
    ...
    for sm in range (200):
        ...
        workbook_res = xlsxwriter.Workbook( "NEpop200/ " + 'NEpop200_under pipe bursts '+ str(sm) +'.xlsx')
