...
def _compress(data, selectors):
    ...
    ### 区别于 python2, python3中 filter, map, zip 本身就已经是 generator了, 故 itertools.izip就显得有点多余了
    #return (d for d, s in itertools.izip(data, selectors) if s)
    ### 解决方案: 将itertools.izip 替换为 zip
...
class Algorithm(object):
    ...
    def run(self, condition):
        ...
        while not condition(self):
            ...
            print (str(self.nfe) + '/' + str(totaltimes))
...
class Archive(object):
    ...
    def add(self, solution):
        ...
        if any(dominates):
            ...
            self._contents = list(_compress(self._contents, nondominated)) + [solution]
            return True
...
class AdaptiveGridArchive(Archive):
    ...
    def add(self, solution):
        ...
        self._contents = list(_compress(self._contents, nondominated))
...
def nondominated_truncate(solutions, size):
    ...
    size: int
        The size of the truncated result
    """
    result = sorted(solutions, key=_cmp_to_key(nondominated_cmp)) 
    return result[:size]
